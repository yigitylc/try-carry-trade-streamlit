...
class TRYHolderParams:
    ...
    def __post_init__(self):
        ...
        # Entry spread (derived from entry rates, for reference)
        if self.spot_entry > 0:
            self.entry_spread = self.entry_bank_rate_try_to_usd / self.spot_entry - 1
        else:
            self.entry_spread = self.exit_spread_pct

        # Spread for exit calculations (use exit_spread_pct)
        ...
        # CRITICAL: Use exit_spread_pct for break-even, NOT entry spread
        self.bank_rate_be = self.final_try / (self.usd_rf_end + self.swift_fee_usd)
        self.spot_be = self.bank_rate_be / (1 + self.exit_spread_pct)
...
class USDHolderParams:
    ...
    def __post_init__(self):
        ...
        # Legacy compatibility
        # CRITICAL: For USD holder, entry_bank_rate should be USD→TRY (below spot)
        # NOT the exit rate TRY→USD (above spot)
        self.entry_bank_rate = self.entry_bank_rate_usd_to_try
        self.usd_rf_rate_annual = self.tbill_rate_annual_usd
...
class TradeParams:
    ...
    # Exit spread for break-even calculation (defaults to entry spread if not set)
    ...
    def __post_init__(self):
        ...
        # Entry spread derived from entry rates
        self.entry_spread = self.entry_bank_rate / self.spot_entry - 1

        # Exit spread (use explicit value or default to entry spread for legacy compat)
        if self.exit_spread_pct is None:
            self.exit_spread_pct = self.entry_spread

        # Spread for backward compatibility (use exit spread for outcome calc)
        self.spread = self.exit_spread_pct
