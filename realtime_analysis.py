...
def compute_realtime_pnl(
    ...
    KEY DISTINCTION:
    - TRY Holder: entry_bank_rate = rate for TRY→USD conversion (above spot)
      usd0_exec = principal_try / entry_bank_rate
    - USD Holder: entry_bank_rate = rate for USD→TRY conversion (below spot)
      usd0_exec = principal_usd (ALWAYS equals input principal)
    """
    # Get spot data
    ...
    # Determine mode and set entry bank rate / usd0_exec / try_principal accordingly
    is_usd_holder = isinstance(params, USDHolderParams)

    if is_usd_holder:
        # USD HOLDER MODE
        # Entry: USD→TRY conversion at user-specified rate
        entry_bank_rate = params.entry_bank_rate_usd_to_try
        # Baseline: original USD principal (NOT derived from TRY)
        usd0_exec = params.principal_usd  # CRITICAL: Always equals input principal
        # TRY received from USD conversion
        try0 = params.principal_usd * params.entry_bank_rate_usd_to_try
        try_principal = try0  # This is the TRY we deposit
    else:
        # TRY HOLDER MODE
        # Entry: TRY→USD conversion at user-specified rate
        entry_bank_rate = getattr(params, 'entry_bank_rate_try_to_usd',
                                  getattr(params, 'entry_bank_rate', entry_spot * (1 + params.spread)))
        # Baseline: USD from converting TRY at entry
        usd0_exec = params.principal_try / entry_bank_rate
        # TRY principal we already have
        try_principal = params.principal_try

    # Current bank rate (EXIT rate - TRY→USD, above spot)
    # This is the same for both modes at exit
    exit_spread = getattr(params, 'exit_spread_pct', params.spread)
    current_bank_rate = current_spot * (1 + exit_spread)

    # Accrued interest (pro-rata on TRY deposit)
    # CRITICAL: Use try_principal (which differs by mode)
    # Calendar day accrual using 365-day year
    accrued_gross = try_principal * params.deposit_rate_annual * (days_elapsed / 365)
    ...
    current_try_value = try_principal + accrued_net

    # If converting now (TRY→USD at current bank rate)
    current_usd_value = current_try_value / current_bank_rate - params.swift_fee_usd

    # Unrealized P&L vs baseline
    unrealized_pnl = current_usd_value - usd0_exec
    unrealized_return = (current_usd_value / usd0_exec - 1) * 100

    # T-Bill benchmark (calendar day accrual using 365-day year)
    # For USD holder: T-bill on principal_usd
    # For TRY holder: T-bill on usd0_exec (converted at entry)
