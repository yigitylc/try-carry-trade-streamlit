...
from dashboard_core import (
    ...
    get_trade_status, compute_rolling_volatility, run_consistency_check
)
...
def main():
    ...
    with st.sidebar:
        ...
        if today >= maturity_date:
            st.warning("**Trade MATURED** - Showing realized values")
    ...
    if not validation['sufficient']:
        st.warning(f"Insufficient data for {regime_name}. Using fallback: {validation['fallback_regime']} days")
