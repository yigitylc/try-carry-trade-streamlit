...
LAYOUT_DEFAULTS = {
    ...
    'margin': {'l': 60, 'r': 120, 't': 60, 'b': 60},  # Increased right margin for annotations
    'hovermode': 'x unified',
}
...
def create_scenario_chart(scenarios_df: pd.DataFrame, params: dict) -> go.Figure:
    ...
    Shows P/L vs Convert Now and Excess vs T-Bill for each spot movement.
    """
    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
    ...
    colors_pnl = [COLORS['profit'] if x >= 0 else COLORS['loss']
                  for x in scenarios_df['pnl_vs_convert_now']]
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=scenarios_df['pnl_vs_convert_now'],
            marker_color=colors_pnl,
            name='P/L vs Convert',
            text=[f"${x:+,.0f}" for x in scenarios_df['pnl_vs_convert_now']],
...
def create_payoff_diagram(params: dict, spot_range: tuple = None) -> go.Figure:
    ...
    fig.add_hline(
        y=rf_return,
        line_dash="dash",
        line_color=COLORS['tbill'],
        annotation_text=f"T-Bill: {rf_return:.2f}%",
    ...
    fig.add_vline(
        ...
        annotation_text=f"Break-even: {spot_be:.2f}",
        annotation_position="top",
    )
    ...
    fig.add_vline(
        ...
        annotation_text=f"Entry: {params['spot_entry']:.2f}",
    ...
    fig.update_layout(
        ...
        yaxis_title='Return vs Convert Now (%)',
        height=500,
        legend=dict(
            orientation='h',
            yanchor='bottom',
...
def create_mc_distribution(mc_results: dict, show_excess: bool = True) -> go.Figure:
    ...
    if show_excess:
        ...
        data = mc_results['ret_vs_convert']
        title = 'Monte Carlo: Return vs Convert Now Distribution'
        xlabel = 'Return vs Convert Now (%)'
        mean_val = mc_results['mean_ret']
        var_95 = mc_results['var_95_ret']
        cvar_95 = mc_results['cvar_95_ret']
        prob_loss = mc_results['prob_loss_vs_convert']
        loss_label = 'Loss'
...
def create_historical_chart(spot_series: pd.Series, params: dict) -> go.Figure:
    ...
    Annotations use xref='paper' to ensure they are never clipped.
    """
    fig = go.Figure()
    ...
    y_min = min(spot_series.min(), params['spot_entry'], params['spot_be']) * 0.98
    y_max = max(spot_series.max(), params['spot_entry'], params['spot_be'], params['entry_bank_rate']) * 1.02

    # Entry spot line (with explicit annotation using xref='paper')
    fig.add_hline(
        y=params['spot_entry'],
        line_dash="dash",
        line_color=COLORS['info'],
    ...
    fig.add_annotation(
        x=1.01, xref='paper',
        y=params['spot_be'], yref='y',
        text=f"Break-even: {params['spot_be']:.2f}",
        showarrow=False,
        font=dict(color=COLORS['breakeven'], size=11),
        xanchor='left',
        bgcolor=COLORS['bg_card'],
    )
    ...
    fig.add_annotation(
        x=1.01, xref='paper',
        y=params['entry_bank_rate'], yref='y',
        text=f"Entry Bank: {params['entry_bank_rate']:.2f}",
        showarrow=False,
        font=dict(color=COLORS['tbill'], size=11),
        xanchor='left',
        bgcolor=COLORS['bg_card'],
    )
...
def create_kpi_cards_html(params: dict, mc_results: dict = None) -> str:
    ...
    """
    usd0 = params['usd0_exec']
    usd_rf = params['usd_rf_end']
    spot_be = params['spot_be']
    be_move = params['be_move_pct']

    cards = [
        ('USD Invested', f"${usd0:,.2f}", 'Executable at entry bank rate', COLORS['info']),
        ...
        ('Bank Spread', f"{params['spread']*100:.2f}%", 'Derived from entry rates', COLORS['text_secondary']),
    ]
...
def export_charts_to_html(
    ...
    <body>
        <h1>{title}</h1>
        <p class="subtitle">Baseline = Convert now at bank rate + T-Bill @ risk-free rate</p>
    ''']
...
def create_cushion_timeline(cushion_data: dict, trend_data: dict) -> go.Figure:
    ...
    if days_to_be < float('inf') and days_to_be > 0:
        if days_to_be > days_remaining:
            ...
            fig.add_annotation(
                x=days_to_be, xref='x',
                y=1.10, yref='paper',
                text=f"DANGER: Hits BE Day {days_to_be:.0f}",
                showarrow=False,
                font=dict(color=COLORS['loss'], size=11, weight='bold'),
                bgcolor=COLORS['bg_card'],
            )
    ...
    fig.update_layout(
        ...
        height=400,
        showlegend=False,
        margin={'l': 60, 'r': 120, 't': 80, 'b': 60},  # Extra top margin for annotations
    )
